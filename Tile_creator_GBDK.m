...
for x=1:1:hor_tile
    for y=1:1:vert_tile
        ...
        if flag==1
            ...
            text(L+1,H+3,dec2hex(pos_identical-1,2),'Color','magenta','FontSize',8)
        else
            rectangle('Position',[L-1/2 H-1/2 8 8],'EdgeColor','g')
            text(L+1,H+3,dec2hex(pos-1,2),'Color','magenta','FontSize',8)
...
for p=1:1:pos
    rectangle('Position',[L-1/2 H-1/2 8 8],'EdgeColor','g')
    text(L+1,H+3,dec2hex(p-1,2),'Color','magenta','FontSize',8)
    L=L+8;
    if L>=memory_width
        L=1;
        H=H+8;
